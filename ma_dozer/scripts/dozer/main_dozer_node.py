...
def main():
    ...
    if config.use_estimated_aruco_pose:
        aruco_position_subscriber = ThreadedSubscriber(topics=[config.topics.topic_estimated_dozer_position],
                                                       ip=config.camera.ip,
                                                       port=config.camera.estimated_position_port,
                                                       callback_func=control_manager.update_pose_aruco)
    else:
        aruco_position_subscriber = ThreadedSubscriber(topics=[config.topics.topic_dozer_position],
                                                       ip=config.camera.ip,
